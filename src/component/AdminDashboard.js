    ...
    const fetchDashboardData = async () => {
      try {
        const [fieldsRes, bookingsRes, managersRes] = await Promise.all([
          axios.get("http://localhost:5000/field", { headers: { Authorization: `Bearer ${user.token}` } }),
          axios.get("http://localhost:5000/booking/today-count", { headers: { Authorization: `Bearer ${user.token}` } }),
          axios.get("http://localhost:5000/auth/count/manager", { headers: { Authorization: `Bearer ${user.token}` } }),
        ]);
