    ...
    const fetchDashboardData = async () => {
      try {
        const [fieldsRes, bookingsRes, managersRes] = await Promise.all([
          axios.get("/field", { headers: { Authorization: `Bearer ${user.token}` } }),
          axios.get("/booking/today-count", { headers: { Authorization: `Bearer ${user.token}` } }),
          axios.get("/auth/count/manager", { headers: { Authorization: `Bearer ${user.token}` } }),
        ]);
