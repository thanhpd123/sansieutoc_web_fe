...
import "../../styles/HomeRoot.css"; // Từ src/Home.css (file gốc)
...
const OwnerFields = ({ user }) => {
    ...
    useEffect(() => {
        ...
        const fetchFields = async () => {
            try {
                const res = await axios.get("http://localhost:5000/field/owner", {
    ...
    const handleDeleteClick = async (id) => {
        ...
        try {
            await axios.delete(`http://localhost:5000/field/${id}`, {
                headers: { Authorization: `Bearer ${user.token}` },
            });
